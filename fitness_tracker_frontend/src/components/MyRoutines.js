...
const MyRoutines = ({loginToken, setGoal, goal, setName, name, setIsPublic, isPublic, user}) => {
  ...
  .catch(console.error);
    ...
    useEffect(() => {fetchMyRoutines(user.username)}, [])
    return <div className='myRoutines'>
    <h1>myRoutines</h1>
    <form onSubmit={(event) => {event.preventDefault()
        createRoutine(name, goal, isPublic)
        fetchMyRoutines()}}><label htmlFor='name'>Name</label>
        <input onChange={(event) => setName(event.target.value)} id='name' required></input>*<br />
        <label htmlFor='description'>Goal</label>
        <input onChange={(event) => setGoal(event.target.value)} id='description' required></input>*<br />
        <label htmlFor='isPublic'>Public?</label>
        <input type="checkbox" onChange={() => isPublic ? setIsPublic(false) : setIsPublic(true)}></input><br />
    ...
    {myRoutines.map((routine) => {
        ...
        <Link to={`/myRoutines/${routine.id}`}><button onClick={() => {
